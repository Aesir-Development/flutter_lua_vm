...
char* vm_exec_func(lua_State* L, const char* func, int argc, Variant* argv) {
    ...
    for (int i = 0; i < argc; i++) {
        Variant v = argv[i];
        switch(v.type) {
            case TYPE_INT: lua_pushinteger(L, v.i);
            case TYPE_DOUBLE: lua_pushnumber(L, v.d);
            case TYPE_STRING: lua_pushstring(L, v.s);
        }
    }


    // TODO - Split result getting result into its own function
